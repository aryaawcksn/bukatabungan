...
export function Sidebar({ activeTab, setActiveTab, user, onLogout, collapsed = false, onMouseEnter, onMouseLeave }: SidebarProps) {
  const menuItems = [
  ...
  return (
    <TooltipProvider>
      <aside 
        className={cn(
          "fixed left-0 top-0 z-40 h-screen border-r border-slate-200 bg-white shadow-sm transition-all duration-300 overflow-x-hidden",
          collapsed ? "w-16" : "w-64"
        )}
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
            <div className={cn(
              "mb-2 px-2 text-xs font-semibold uppercase tracking-wider text-slate-400 transition-all duration-300 whitespace-nowrap overflow-hidden",
              collapsed ? "opacity-0 invisible h-0 mb-0" : "opacity-100 visible h-auto"
            )}>
              Menu Utama
            </div>
            ...
            {collapsed ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    onClick={onLogout}
                    className="group flex w-full items-center justify-center rounded-lg py-2.5 px-0 text-sm font-medium text-red-600 transition-all hover:bg-red-50 hover:text-red-700"
                  >
